...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	%     The {\itshape tactic} \lstinline! intros!.        
	     ...
	     In order to proof theorem \lstinline!plus_id_example! the theorem's hypothesis is rewritten as shown in listing \ref{lst:plus_id_example}.
	     \begin{lstlisting}[caption = \lstinline!plus_id_example! and it's proof, label= {lst:plus_id_example}]
           Theorem plus_id_example: for all n m: nat,
             n = m ->
             n + n = m + m.
           Proof.
             ...
             Qed.
         \end{lstlisting}
         For a demonstartion of the usage of the Coq-code listed in listing \ref{lst:plus_id_example}. 
         This Coq-Code's translation to a mathematical proof is listed in table \ref{tab:aMathematicalTranslation}.  
         \begin{table}[h]       
	   	  \begin{center}
	 	    \begin{tabular}{|c|l|l|}	       
	     	\hline
	     	 line no.  & a mathematical translation           & comment  \\  \hline
	     	  ...
	     	  2         & $n=m$                                & with the name   \\ 
	          3        & $\quad\implies P(n,m) := n+n = m+m.$      & \lstinline!plus_id_example! in Coq \\  
	          ...
	          6         & $H(n,m) :=\{n=m\}.$                 & Moves the hypothesis \lstinline!H! \\ 
	    	                ...
	    	                &                                     & focus of Coq.\\ \hline 
	          7         & $H(n,m)\implies $                   & Substitute by H from \\  
	     	            & $\implies( P(n,m)\Leftrightarrow\{m+m = m+m\}).$
	     	                    	                              & left to right in the \lstinline!subgoal!.\\
                        &     	                              & Simplify if possible. \\ \hline
	     	  8         & trivial.                             & We obtained a                  \\ 
	     	            &                                      & trivial statement. \\ \hline
	         ...
	         \end{center}          
          \end{table}  
       N	ote that the semantics of the Coq-code in listing \ref{lst:plus_id_example} can be easily understood by a reader who is familiar with the notation of predicate logic as in \ref{}. 
       The function of the tactics reflexivity is also similar to princliple of refelxifvity in predicate logic. 
       Therefore a one-to-one taranslation of this listing in predicate logic is listen in appendix \ref{app:AdditionalMaterials} \ref{subsec:CoqAndPredicateLogic}.  \\      
       % Steffens Skrip.    
