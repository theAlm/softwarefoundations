  ...
  \end{lstlisting}
   ...
   \begin{tabular}{|c|c|c|c|}
     \hline 
 	  ...
 	  x - y         & minus           & substract two natural numbers       & see listing  \ref{lst:minus_nat} \\  \hline
   ...
   Let's recall what relfexivity in terms of an equivalence relation means.  
   ...
   \item $a \thicksim b$ if and only if $b \thicksim c$ (symmertry)
...
%     The {\itshape tactic} \lstinline! intros!.     
     ...
     \begin{tabular} {|l|l|l|}
     	\hline
     	 ...
     	 \lstinline!proof.!              & $\text{wts}: \forall m,n \in \mathbb{N}:$ & Moves the last used \\   
       	                                 ...
       	                                 &                                     &                                    \\   \hline
         \lstinline!  intros n m.!       & Assume $m,n \in \mathbb{N}.$        & Moves the universally quanitified variables\\
   ...
   \end{lstlisting}   
	...
	The comand \lstinline!Admitted! tells Coq to skip a proof of a theorem and to accept it as given.
	It can be used in long proofs to subsidary state longer \lstinline!Lemmas!.
	If a proof was startet it can be interuped by the command \lstinline!Abort!.
	Note, if a proof was forgotton in the follwing nonesense is a able to be shown. \\
	
	
 \subsection{Proof by Case Analysis}
   Consider the follwing example. It clearly demonstartes that, it is not possible to prove everyting using the simplification tactic.   
