...
\subsection{Numbers}
  ...
  \begin{example}
	  ...
	  \end{tabular}
  \end{example}
  Moreover, it is easy to see if $n$ is in the set \lstinline! nat!, it follows \lstinline! S n ! is an expression belonging to the set \lstinline! nat !.
  And expressions belong the set \lstinline! nat! if and only if they are  formed by either of those methods (i.e. \lstinline! 0 ! or  \lstinline! S n!). 