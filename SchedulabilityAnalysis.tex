...
In this section we want to investigate the encapsulated EDF-scheduler (earliest deadline first scheduler) from the schedulig procedure by Kaiser \cite{K}. It is homogenous multiprocessor systems comprising processors.\\

Let's recap \glspl{real-time system} as in \cite{KBK}.
The {\itshape process} is defined as an sequential execution of a program on a processor.
...
The slotted prirority modell from \cite{B97} with  sporadic and periodic processes (\cite{K}).
...
Moreover there is this requirement labeld by  $\mathcal{B}$.
\begin{quote}
...
\begin{align}
 & \delta_p \in \mathbb{N} \quad \text{be a periodic disruptive process and}\\
 	...
 	&\delta e_i \in \mathbb{N} \quad \text{for} \quad  i = 1,..,n \quad  \text{denote the execution time}.  
\end{align}   
\end{definition}
...
According to \cite[]{B97} due to the establishing the exclusion model we denote by
