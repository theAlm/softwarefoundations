...
This is a summary of the electronic text-book \cite{PACGGHSY} with comments and a little rewritten sections by the author and based on the peers and supervisor's feedback. Furthermore, applictaions of the Coq proof assitent were added. Other references are marked. 
...
\begin{itemize}
	\item basics concepts of logic (see Appendix \ref{app:AdditionalMaterials} \ref{subsec:CoqAndPredicateLogic}) 
	\item functional programming in Coq (see Section \ref{sec:BasicFunctionalProgrammingInCoq})
	\item compiling using Coq (see Section \ref{sec:importing})
	\item applying inductive proofs using coq (see Section \ref{sec:Induction}) 
	\item working with inductive type defintions (see Section \ref{sec:ListsWorkingWithStructuredData})
	\item schedulability anylysis modeling (see Section \ref{sec:ScheduabilityAnalysisModeling})
	\item high order logic and model checking (see Section \ref{sec:HighOrderLogicAndModelChecking})
	\item introduction to RT-proofs (see Section \ref{sec:IntroductionToRTProofs})
\end{itemize} 
