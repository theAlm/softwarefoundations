...
\begin{itemize}
%
\item $a \in M$ \iffd\ $a$ ist ein Element der Menge $M$\index{$\in$}
%
\item $a \not\in M$ \iffd\ $a$ ist kein Element der Menge $M$\index{$\not\in$}
%
...
\not\in B}$ (Differenz von $A$ und $B$)\index{$\backslash$}
%
\item $\overline{A} \eqd M \backslash A$ (Komplement von $A$ 
bezüglich einer festen Grundmenge $M$)\index{$\overline{A}$}
%
...
\begin{itemize}
  % 
  \item $(\enu{a}{1}{n}) \eqd$ die Elemente $\enu{a}{1}{n}$ in dieser
  festgelegten \emph{Reihenfolge} und z.B.~$(3,2) \not= (2,3)$. Wir
  sprechen von einem $n$-Tupel.
  % 
  \item $A_1 \times A_2 \times \dots \times
  A_n \eqd \set{(\enu{a}{1}{n}) \colon a_1 \in A_1, a_2 \in A_2, \dots
  ,a_n \in A_n }$ (Kreuzprodukt der Mengen $A_1, A_2, \dots ,
  A_n$)\index{Kreuzprodukt}\index{$A_1, A_2, \dots , A_n$},
  %
...
Mengen, eine Reihenfolge vorgegeben, d.h.~es gilt z.B.~immer
$\set{a,b} = \set{b,a}$, aber im Allgemeinen $(a,b) \not= (b,a)$.
...
Informatik \emph{Permutation von endlichen Mengen} benötigt.
