...
könne. Diese Einstellung zeigt sich oft auch darin, dass Programme mit
Hilfe einer IDE schnell "`testweise"' übersetzt werden, in der
Hoffnung oder (schlimmer) in der Überzeugung, dass ein übersetzbares
...
Beweistechniken haben sollte. Interessanterweise zeigt die Erfahrung im
...
Aus diesen Gründen sollen in diesem Abschnitt einige grundlegende
...
Dieser Beweis war nur wenig formal, aber sehr ausführlich und wurde durch
das Symbol "`$\#$"' beendet. Im Laufe der Zeit hat es sich eingebürgert, 
das Ende eines Beweises mit einem besonderen Marker abzuschließen. 
Besonders bekannt ist hier "`$\mathrm{qed}$"'\index{$\mathrm{qed}$}, 
eine Abkürzung für die lateinische Floskel "`quod erat demonstrandum"', 
die mit "`was zu beweisen war"' übersetzt werden kann. In neuerer Zeit 
...
Aussagen können oft auf verschiedene Weise interpretiert werden. Diese
...
wenn"' durch \dindex{\gdw} abgekürzt.} verknüpft sind. In diesem Fall
...
\noindent "`$\Rightarrow$"': Wenn $n$ durch $3$ teilbar ist und $\sum_{i=0}^k
a_i \cdot 10^i= n$, dann müssen alle Koeffizienten $a_i$ durch $3$
teilbar sein, denn $10^i$ ist kein Vielfaches von $3$. Dann ist aber die
Quersumme $\sum_{i=0}^k a_i$ der Dezimaldarstellung von $n$ sicherlich
durch $3$ teilbar.

\medskip

\noindent "`$\Leftarrow$"': Dieser Fall soll durch Kontraposition gezeigt
werden. Sei nun $n$ nicht durch $3$ teilbar, dann muss es in der
Dezimaldarstellung von $n$ mindestens ein Koeffizienten $a_i$ geben,
der auch nicht durch $3$ teilbar ist. Damit kann die Quersumme
$\sum_{i=0}^k a_i$ von $n$ aber kein Vielfaches von $3$ sein, was
zeigt, dass $3$ kein Teiler der Quersumme ist.\qed
\end{proof}
...
auf den ersten Blick sehr kompliziert erscheint, ist sie oft einfach
...
\dindex{Induktionsprinzip}. Wir wollen jetzt nachweisen, dass für jedes $n \in
\N$ eine bestimmte Eigenschaft $E$ gilt. Wir schreiben kurz $E(n)$ für
die Aussage "`$n$ besitzt die Eigenschaft $E$"'. Mit der Schreibweise
$E(0)$ drücken\footnote{Mit $E$ wird also ein Prädikat bezeichnet
(siehe Abschnitt \ref{MengenDef})} wir also aus, dass die erste
natürliche Zahl $0$ die Eigenschaft $E$ besitzt.
